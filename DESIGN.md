...
The `letters-phonemic/` set (26 clips, one phonemic sound per letter) remains in the bundle — it voices `hide`'s found-impostor announcement in speak mode — but is no longer the unknown-word fallback.

All three libraries are rendered and committed; `verify_clips.py` checks the full tree. The phoneme clips are mid-ear-tuning: some run long, especially the vowels. `tools/tts/test_phonemes.py` concatenates CMU-known words from the clips and lets you A/B them against the pre-rendered word clips by ear. Concatenated playback won't match co-articulated speech, but intelligibility is the bar, and the constants above (plus each phoneme's `PHONEME_MAP` spelling) are the knobs — delete a clip and re-run the generator to re-render just that one.
